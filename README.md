...
- **secrets** — External Secrets Operator + **OpenBao** (open-source Vault), nothing
  sensitive in git
- **CI/CD** — GitHub Actions builds/tests/scans, and commits dev's own tag bump
  directly; **qa/prod are promoted by a gated, reviewable workflow**, not by hand
...
| App namespaces | `user-management-app-dev-ns` (armed/auto), `-qa-ns` (PR auto-merges in 15m, 1h reconcile), `-prod-ns` (PR-gated, environment-approved, manual merge) |
| Platform namespaces | `monitoring`, `external-secrets`, `openbao`, `cnpg-system`, `flux-system` |
| Secrets | values only in OpenBao — `kv/monitoring/{gmail-smtp,grafana-admin}`, `kv/user-management-app/jwt-{dev,qa,prod}` (one per env, never shared); Postgres creds generated by CNPG |
...
```
 ...
 GitHub Actions: ci.yml (test+build+push on every merge to main, tags dev-<run>/main-<sha>,
                     then commits dev's own overlay bump directly — no PR, nothing gates dev)
...
.github/workflows/
  ci.yml                      validate manifests, unit+integration test the API, build (+push on main),
                               then bump-dev commits dev's own overlay tag directly (no PR)
...
(API — one independent secret per environment: `jwt-dev`/`jwt-qa`/`jwt-prod`, never
shared). Postgres credentials are generated by CloudNativePG, not OpenBao. CI/CD needs
no OpenBao secret of its own — every git-write (dev's bump, qa/prod's overlay PRs) uses
the default `GITHUB_TOKEN`, scoped to the job, provided free by GitHub Actions.
...
**Ordering (`dependsOn`):** `prometheus` → `grafana-operator` → `grafana` → `observability`;
`prometheus` → `sloth`; `cert-manager` → `cnpg-operator` (the Barman Cloud plugin it
applies needs cert-manager's webhook already running);
`external-secrets-operator` + `openbao` → `secrets` → `grafana` + `alert`;
`cnpg-operator` + `secrets` → `user-management-app-{dev,qa,prod}`.
...
**`openbao` / `secrets` (and thus `grafana`, `alert`) stay not-ready until you
bootstrap OpenBao once.** Full command block:
...
then run **Promote to dev** once to arm it. `ci.yml`'s `bump-dev` job takes it from
there on every push to `main` — no further manual step.
...
push to main
   ...
   │         → bump-dev commits the bump directly into
...
"Promote to qa" (workflow_dispatch, version: vX.Y.Z)
   ...
   │  auto-merges itself 15 minutes after opening — close the PR (or push a fix to
   │  its branch) within that window to stop it; no action needed to let it through
   ▼
...
No separate git-write credential to set up anywhere — `bump-dev` (in `ci.yml`) and
`promote-qa`/`promote-prod`'s own overlay-bump commits all use the default
`GITHUB_TOKEN`, scoped to the job, provided automatically by GitHub Actions.
...
|---|---|---|
| **dev** | `bump-dev` committed a bad `dev-<N>` tag | `git log --oneline --author=github-actions -5` → `git revert <sha> && git push`. Flux reconciles the revert within 5m. |
...
(GitHub Actions builds/tests/scans and bumps dev directly; gated PR promotion to
qa/prod), as documented above.
